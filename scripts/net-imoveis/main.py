    ...
    try:
        ...
        while not page_ready and attempt < max_attempts:
            try:
                ...
                if "{{" in page_source and "}}" in page_source:
                    print(
                        f"Página {page_number} ainda tem templates não renderizados. Aguardando..."
                    )
        ...
        for imovel in imoveis:
            try:
                ...
                descricao_raw = (
                    imovel.select_one("h2").text if imovel.select_one("h2") else ""
                )
                ...
                if descricao_raw:
                    ...
                    descricao = ""
                address = (
                    imovel.select_one("div.endereco").text.strip()
                    if imovel.select_one("div.endereco")
                    else ""
                )
                m2 = (
                    imovel.select_one("div.caracteristica.area").text.split()[0]
                    if imovel.select_one("div.caracteristica.area")
                    else ""
                )
                bedroom = (
                    imovel.select_one("div.caracteristica.quartos").text.split()[0]
                    if imovel.select_one("div.caracteristica.quartos")
                    else ""
                )
                vagas_estacionameto = (
                    imovel.select_one("div.caracteristica.vagas").text.split()[0]
                    if imovel.select_one("div.caracteristica.vagas")
                    else ""
                )
                bathroom = (
                    imovel.select_one("div.caracteristica.banheiros").text.split()[0]
                    if imovel.select_one("div.caracteristica.banheiros")
                    else ""
                )
                price = (
                    imovel.select_one("div.valor").text.split()[1]
                ...
                for key, value in property_data.items():
                    if isinstance(value, str) and ("{{" in value or "}}" in value):
                        has_template = True
                        print(
                            f"Pulando imóvel com templates não renderizados na página {page_number}"
                        )
                        break
...
def scrape_netimoveis(
    ...
    def get_driver():
        ...
        if not hasattr(thread_local, "driver"):
            ...
            chrome_options.add_argument(
                "--host-resolver-rules=MAP *.plausible.io 127.0.0.1"
            )
    ...
    def save_batch_data(batch_properties):
        ...
        if (
            ...
            data_handler.save_to_excel(
                batch_df, excel_path, append=True
            )  # Sempre append=True para lotes
            data_handler.save_to_tsv(
                batch_df, tsv_path, append=True
            )  # Sempre append=True para lotes
            ...
            if property_type:
                file_prefix = (
                    f"imoveis_netimoveis_{tipo_transacao.lower()}_{property_type}"
                )
            else:
            ...
            data_handler.save_to_excel(
                batch_df, excel_filename, output_dir=output_dir, append=True
            )  # Sempre append=True para lotes

            # Salvar em CSV
            csv_filename = f"{file_prefix}.csv"
            data_handler.save_to_csv(
                batch_df, csv_filename, output_dir=output_dir, append=True
            )  # Sempre append=True para lotes

            print(
                f"Batch data saved to {output_dir}/{excel_filename} and {output_dir}/{csv_filename}"
            )
    ...
    if output_dir is not None and not append:
        ...
        if (
            ...
            empty_handler.save_to_excel(
                empty_df, custom_output_files["excel_path"], append=False
            )
            empty_handler.save_to_tsv(
                empty_df, custom_output_files["tsv_path"], append=False
            )
        else:
            ...
            empty_handler.save_to_excel(
                empty_df, excel_filename, output_dir=output_dir, append=False
            )
            empty_handler.save_to_csv(
                empty_df, csv_filename, output_dir=output_dir, append=False
            )
        ...
        while continue_scraping and page <= total_pages:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if status_code == 200 and properties:
                    ...
                    print(
                        f"Obtidos {len(properties)} imóveis. Total até agora: {len(all_properties)}"
                    )
                else:
            ...
            if batch_properties and output_dir is not None:
                ...
                print(
                    f"Salvando dados do lote: páginas {current_batch_start}-{current_batch_end}..."
                )
            ...
            if page <= total_pages and batch_delay:
                current_batch_end = page - 1
                print(
                    f"Lote processado: páginas {current_batch_start}-{current_batch_end}. Aguardando {batch_delay} segundos antes do próximo lote..."
                )
...
def run_scraper(
    ...
    try:
        ...
        PROPERTY_TYPES = ["apartamento", "casa", "flat"]

        # Validate property_type if provided
        if property_type and property_type not in PROPERTY_TYPES:
            print(
                f"Warning: '{property_type}' is not a recognized property type. Using default URL without property type filter."
            )
            property_type = None
...
def run_all_scrapers(
    ...
    consolidated_dfs = {"venda": [], "aluguel": []}
    ...
    for contract_type in contract_types:
        for property_type in property_types:
            ...
            if df is not None and not df.empty:
                ...
                print(
                    f"Added {len(df)} rows from {property_type} to consolidated {contract_type} data"
                )
    ...
    for contract_type, df_list in consolidated_dfs.items():
        if df_list:
            ...
            data_handler.save_to_excel(
                full_df, excel_filename, output_dir=output_dir, append=append
            )
            data_handler.save_to_csv(
                full_df, csv_filename, output_dir=output_dir, append=append
            )

            print(
                f"Saved consolidated {contract_type} data to {output_dir}/{excel_filename} and {output_dir}/{csv_filename}"
            )
