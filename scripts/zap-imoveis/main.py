...
def extrair_dados():
    ...
    for imovel in imoveis:
        try:
            descricao = imovel.find_element(
                By.CLASS_NAME, "ListingCard_card__description__slBTG"
            ).text
        except:
            descricao = None
        ...
        try:
            preco = imovel.find_element(
                By.CSS_SELECTOR, 'p[data-cy="rp-cardProperty-price-txt"]'
            ).text
        except:
            preco = None

        try:
            area = imovel.find_element(
                By.CSS_SELECTOR, 'p[data-cy="rp-cardProperty-propertyArea-txt"]'
            ).text
        except:
            area = None

        try:
            quartos = imovel.find_element(
                By.CSS_SELECTOR, 'p[data-cy="rp-cardProperty-bedroomQuantity-txt"]'
            ).text
        except:
            quartos = None

        try:
            banheiros = imovel.find_element(
                By.CSS_SELECTOR, 'p[data-cy="rp-cardProperty-bathroomQuantity-txt"]'
            ).text
        except:
            banheiros = None

        try:
            vagas = imovel.find_element(
                By.CSS_SELECTOR,
                'p[data-cy="rp-cardProperty-parkingSpacesQuantity-txt"]',
            ).text
        except:
            vagas = None

        try:
            endereco = imovel.find_element(
                By.CSS_SELECTOR, 'h2[data-cy="rp-cardProperty-location-txt"]'
            ).text
        except:
            endereco = None
...
max_scroll_per_page = (
    40000  # Limite de rolagem por página antes de mudar para a próxima página
)
...
while len(todos_dados_imoveis) < target_total:
    ...
    while scroll_total < max_scroll_per_page:
        ...
        todos_dados_imoveis = list(
            {frozenset(item.items()): item for item in todos_dados_imoveis}.values()
        )  # Remove duplicatas globais
        print(
            f"Total de imóveis coletados até agora: {len(todos_dados_imoveis)} / {target_total}"
        )
    # Resetando o contador de scroll e tentando avançar para a próxima página
    scroll_total = 0
    try:
        next_button = driver.find_element(
            By.CSS_SELECTOR, 'button[data-testid="next-page"]'
        )
