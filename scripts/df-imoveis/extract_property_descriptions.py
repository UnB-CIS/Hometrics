    ...
    while retries <= max_retries:
        try:
            ...
            if response.status_code == 200:
                ...
                description_elements = property_soup.find_all('p', class_='texto-descricao')
                
                if description_elements:
                    # Extract and clean the description text from all matching elements
                    full_description = ' '.join([clean_description(elem.text) for elem in description_elements])
...
def extract_property_descriptions(category, tsv_filename=None, output_dir=None, 
                                delay_min=0.2, delay_max=0.5, max_properties=None, 
                                workers=6, batch_size=50, save_each_batch=True):
    """
    ...
    try:
        ...
        if max_properties and max_properties < total_properties:
            print(f"Limiting processing to {max_properties} out of {total_properties} properties")
        ...
        num_batches = (total_properties + batch_size - 1) // batch_size  # Ceiling division
        ...
        for batch_idx in range(num_batches):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for future in tqdm(concurrent.futures.as_completed(future_to_idx), 
                    ...
                    try:
                        ...
                        if (not description or description.strip() == "") and status == 'success':
                            print(f"[WARN] Empty description for idx {idx}, url: {url}. Retrying after 10s...")
                            time.sleep(10)
                            description_retry, status_retry = fetch_property_description(url, HEADERS)
                            if description_retry and description_retry.strip() != "":
                                description = description_retry
                                status = status_retry
                                print(f"[INFO] Retry succeeded for idx {idx}")
                            else:
                                print(f"[ERROR] Still empty after retry for idx {idx}, url: {url}")

                        if status == 'success' and description and description.strip() != "":
                            batch_results.append((idx, description))
                            success_count += 1
                        else:
                            print(f"[ERROR] Failed to fetch description for idx {idx}, url: {url}, status: {status}")
                            failure_count += 1
                    except Exception as e:
                    ...
                    if processed_count % pause_every == 0:
                        ...
                        print(f"[HUMAN-LIKE] Pausing for {micro_pause:.2f}s to look at details...")
            ...
            if save_each_batch:
                temp_output_path = os.path.join(output_dir, f"imoveis_df_{category}.tsv")
                df.to_csv(temp_output_path, sep='\t', index=False)
                print(f"Saved interim results to {temp_output_path}")
            
            if batch_idx < num_batches - 1:  
                batch_delay = random.lognormvariate(2.5, 0.4)  # mean ~12s, still with some jitter
                print(f"[HUMAN-LIKE] Waiting {batch_delay:.2f} seconds before next batch...")
