...
def extrair_dados():
    ...
    for anuncio in anuncios:
        ...
        try:
            area = anuncio.find_element(
                By.CLASS_NAME, "property-card__detail-area"
            ).text
        except:
            area = "N/A"

        try:
            quartos = anuncio.find_element(
                By.XPATH, ".//ul[@class='property-card__details']/li[1]"
            ).text
        except:
            quartos = "N/A"

        try:
            banheiros = anuncio.find_element(
                By.XPATH, ".//ul[@class='property-card__details']/li[2]"
            ).text
        except:
            banheiros = "N/A"

        try:
            vagas = anuncio.find_element(
                By.XPATH, ".//ul[@class='property-card__details']/li[3]"
            ).text
        except:
            vagas = "N/A"
        ...
        try:
            amenidades = anuncio.find_element(
                By.CLASS_NAME, "property-card__amenities"
            ).text
        except:
            amenidades = "N/A"
...
def definir_ranges_vivareal(preco_inferior=1, preco_superior=250000):
    ...
    while totais_ranges < 0.995 * total:
        ...
        driver.get(url)
        time.sleep(10)
        total_range_atual = int(
            WebDriverWait(driver, 10)
            .until(
                EC.presence_of_element_located(
                    (By.CLASS_NAME, "results-summary__count")
                )
            )
            .text.replace(".", "")
        )
        if total_range_atual > 9950:
            ...
            print(
                f"{[preco_inferior,preco_superior]} tem {total_range_atual}, adicionando à lista"
            )
...
for rangezinho in ranges:
    ...
    total_paginas = (total_imoveis // imoveis_por_pagina) + (
        1 if total_imoveis % imoveis_por_pagina != 0 else 0
    )
            ...
            try:
                next_button = WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable(
                        (
                            By.XPATH,
                            "//button[@class='js-change-page' and @title='Próxima página']",
                        )
                    )
                )
...
total_imoveis = int(
    WebDriverWait(driver, 10)
    .until(EC.presence_of_element_located((By.CLASS_NAME, "results-summary__count")))
    .text.replace(".", "")
)
