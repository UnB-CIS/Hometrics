...
library(pacman)
pacman::p_load(tidyverse, stats, DataExplorer, GGally, plotly)
...
# Chamando Funções de filtragem ================================================

data_cleaned <- get_censorship(data = data_cleaned, var = "price", pct = 0.05)
data_cleaned <- get_censorship(data = data_cleaned, var = "size", pct = 0.05)
data_cleaned <- get_remove_outliers(data = data_cleaned)

# Sumarizando os dados limpos e organizados 

sumarizando = data_cleaned  %>% 
...
# Visuliazando a distribuição da variável target \* 

ggplot2::ggplot(data_cleaned) +
          aes(x = price_m2, fill = property_type) +
          geom_histogram(bins = 30L) +
          scale_fill_brewer(palette = "Blues", direction = 1) +
          labs(title = "Distribuição do preço de imóveis no DF", 
               y = "Contagem", 
...
ggplot2::ggplot(sumarizando) +
          aes(x = property_type, y = value, fill = name) +
